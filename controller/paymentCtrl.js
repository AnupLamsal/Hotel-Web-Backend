...
const deletePaymentByUser = async (req, res) => {
  const { id } = req.params;
  try {
...
const sold = async (id, quantity, oldSold) => {
  ...
  if (!quantity)
    return res.status(400).json({ message: "Quantity does not exist." });
  await foodPage.findOneAndUpdate(
    { _id: id },
    {
...
const getStatus = async (req, res) => {
  try {
    ...
    if (paymentFood.status === false) {
      await paymentPage.findOneAndUpdate(
        { _id: req.params.id },
        {
          status: true,
        },
