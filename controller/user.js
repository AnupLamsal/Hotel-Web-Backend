    ...
    if (existingUser.role === 1) {
      res.status(200).json({
        result: {
          ...
          selectedFile: existingUser.selectedFile,
        },
        token,
        message: `Welcome Admin, ${existingUser.name.split(" ")[0]}`,
      });
    } else {
      res.status(200).json({
        result: {
...
const signup = async (req, res) => {
  ...
  try {
    ...
    if (address.length < 3 || address.length > 20)
      return res.status(404).json({ message: "Address required 3 to 20 char" });
...
const getVerified = async (req, res) => {
  ...
  try {
    ...
    if (!Verified)
      return res.status(400).send({ message: "Verification expire re-login" });
...
const updateSingleUser = async (req, res) => {
  ...
  try {
    ...
    // selectedFile validation
    if (selectedFile === null)
      return res.status(404).json({ message: "SelectedFile is Required" });
    ...
    const token = jwt.sign(
      { email: updatedUser.email, id: updatedUser._id, role: updatedUser.role },
      process.env.JWT,
      { expiresIn: "1d" }
    );
    ...
    res
      .status(200)
      .json({ result: updatedUser, token, message: "User Updated" });
  } catch (error) {
    res.json({ message: error });
  }
};
...
const reportData = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(400).json({ message: "User does not exist." });
    const report = user.report.find(
      item => item.reporterUserId === reportData.reporterUserId
    );
    if (report)
      return res
        .status(404)
